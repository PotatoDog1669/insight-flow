...
说明：品牌名为 **Insight Flow**；代码内仍有少量历史兼容标识（如历史兼容数据库名、部分默认文案）属于当前实现现状。

## 2) 仓库目录结构（backend/frontend/docs/scripts）与职责
...
- `postgres`：`postgres:16-alpine`，DB 使用历史兼容数据库名（内部兼容标识）
