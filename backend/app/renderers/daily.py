...
CATEGORY_ORDER = ["模型发布", "产品应用", "技术与洞察", "行业动态", "前瞻与传闻"]
...
def _build_event(item: ProcessedArticle, index: int) -> dict:
    ...
    published_at = item.raw.published_at.isoformat() if isinstance(item.raw.published_at, datetime) else None
...
def _build_detail(item: ProcessedArticle) -> str:
    content = (item.raw.content or "").strip()
    if content:
