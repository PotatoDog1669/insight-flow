"""Codex agent client helpers (OAuth/API-key auth + JSON response parsing)."""
...
def build_codex_headers(config: dict | None = None) -> dict[str, str]:
    cfg = dict(config or {})
    auth_mode_raw = str(cfg.get("auth_mode") or settings.codex_auth_mode or "api_key").strip().lower()
    auth_mode = auth_mode_raw if auth_mode_raw in {"api_key", "oauth"} else "api_key"

    if auth_mode == "oauth":
        token = str(cfg.get("oauth_token") or settings.codex_oauth_token or "").strip()
    else:
        token = str(cfg.get("api_key") or settings.codex_api_key or settings.openai_api_key or "").strip()

    if not token:
        raise ValueError(f"Missing codex credential for auth_mode={auth_mode}")
...
async def run_codex_json(prompt: str, config: dict | None = None) -> dict:
    ...
    timeout_sec = float(cfg.get("timeout_sec") or settings.codex_timeout_sec or 90)
...
async def _post_responses_with_fallback(client: httpx.AsyncClient, base_url: str, payload: dict) -> httpx.Response:
    ...
    if last_error:
        raise last_error
    raise RuntimeError("No codex response endpoint available")
