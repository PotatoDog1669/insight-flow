...
@pytest.mark.asyncio
async def test_run_paper_review_with_retry_uses_fallback_provider_after_failure() -> None:
    calls: list[str] = []
    ...
    class _FallbackProvider:
        async def run(self, payload: dict, config: dict | None = None) -> dict:
            ...
            return {"digest_title": "ok", "digest_summary": "done", "papers": payload["papers"]}

    def _get_provider(*, stage: str, name: str):  # noqa: ANN202
        assert stage == "paper_review"
        if name == "llm_openai":
    ...
    result, provider_name = await run_paper_review_with_retry(
        ...
        provider_overrides={},
        payload={"papers": [{"paper_identity": "2603.12345", "title": "World Model Policy"}]},
        provider_getter=_get_provider,
    )

    assert provider_name == "llm_backup"
    assert result["digest_summary"] == "done"
    assert calls == ["primary", "fallback"]


@pytest.mark.asyncio
async def test_run_paper_review_with_retry_reraises_unavailable_llm_openai() -> None:
    calls: list[str] = []
    ...
    with pytest.raises(ProviderUnavailableError, match="auth_failed"):
        await run_paper_review_with_retry(
            route=StageRoute(primary="llm_openai", fallback=["llm_backup"]),
            providers={
